...
WEAPON("coathanger", (char *)0,
	1, 1, 0,  0,  5,  2,  3,  3, 2, P,   P_DAGGER, IRON, HI_METAL),
...
#ifdef KOPS
WEAPON("letter of expulsion", (char *)0,
...
HELM("plastic hard hat", "hard hat",
		0, 0,  0,	3, 1, 40,  10, 8, 0, PLASTIC, CLR_YELLOW),
HELM("fedora", (char *)0,
...
CLOAK("CECSish cloak", "coarse mantelet",
...
SHIELD("CECS shield", "ham-fisted shield",
...
RING("hawtness", ADORNED, "wooden",         100, 1, 1, 2, WOOD, HI_WOOD),
...
RING("flamebait", CONFLICT, "ruby",         300, 1, 0, 9, GEMSTONE, CLR_RED),
...
RING("free action",     FREE_ACTION, "ooh shiny",
...
AMULET("amulet of Ranma 1/2",     "square",     0,          130),
						/* POLYMORPH */
AMULET("amulet of frigidity",     "austere",    FRIGID,     100),
...
CONTAINER("coat rack", (char *)0,	1, 0, 0,   0,100,  24, WOOD, HI_WOOD),
...
TOOL("WatCard", (char *)0,  1, 0, 0, 0,  15,  1,  10, PLASTIC, CLR_WHITE),
#else
...
TOOL("prosthetic arm", "human arm",
				0, 0, 0, 0,   0,200,  30, IRON, CLR_BROWN),
/* traps */
...
FOOD("foie gras",            0, 1,  1, 0, FLESH, 600, CLR_BRIGHT_MAGENTA),
FOOD("monkey head",          0, 1,  1, 0, FLESH, 100, CLR_BROWN),
FOOD("placenta",             0, 1, 20, 0, FLESH, 300, CLR_BROWN),
...
FOOD("luckee cookie",       55, 1,  1, 0, VEGGY,  40, CLR_YELLOW),
...
POTION("confusion", "orange",           1, CONFUSION,  41, 100, CLR_ORANGE),
POTION("blindness", "yellow",           1, BLINDED,    40, 150, CLR_YELLOW),
POTION("paralysis", "emerald",          1, 0,          41, 300, CLR_BRIGHT_GREEN),
POTION("speed", "dark green",           1, FAST,       42, 200, CLR_GREEN),
POTION("levitation", "loganberry",      1, LEVITATION, 42, 200, CLR_CYAN),
POTION("LSD", "sky blue",               1, HALLUC,     40, 100, CLR_CYAN),
...
POTION("healing", "purple-red",         1, 0,          56, 100, CLR_MAGENTA),
POTION("extra healing", "sexy",         1, 0,          46, 100, CLR_RED),
...
POTION("object detection", "smoky",     1, 0,          41, 150, CLR_GRAY),
POTION("gain energy", "cloudy",         1, 0,          41, 150, CLR_WHITE),
POTION("sleeping",  "effervescent",     1, 0,          41, 100, CLR_GRAY),
POTION("full healing",  "black",        1, 0,          10, 200, CLR_BLACK),
POTION("polymorph", "golden",           1, 0,          10, 200, CLR_YELLOW),
POTION("vodka", "white and goopy",      0, 0,          41,  50, CLR_BROWN),
POTION("sickness", "fizzy",             0, 0,          41,  50, CLR_CYAN),
POTION("fruit juice", "dark",           0, 0,          41,  50, CLR_BLACK),
POTION("acid", "white",                 0, 0,          10, 250, CLR_WHITE),
POTION("oil", "murky",                  0, 0,          30, 250, CLR_BROWN),
POTION("hydrofluoric acid", "teflon",   0, 0,          10, 250, CLR_CYAN),
...
#define SCROLL(name,text,mgc,prob,cost) OBJECT( \
	...
	SCROLL("enchant armor",         "HEY HEY RQ",           1,  63,  80),
	SCROLL("destroy armor",         "NDA SIG NHERE",       1,  45, 100),
	SCROLL("confuse monster",       "CELICIOUS DAKE",       1,  53, 100),
	SCROLL("scare monster",         "OHGO DTHIS ISECKS",    1,  35, 100),
	SCROLL("remove curse",          "BHAVYA",               1,  65,  80),
	SCROLL("enchant weapon",        "DAIYEN FOOELS",        1,  80,  60),
	SCROLL("create monster",        "HF GL DD GG",          1,  45, 200),
	SCROLL("taming",                "PIXAR ITDIN TAPPEN",   1,  15, 200),
	SCROLL("eliminate",             "PIKDUM YLOH",          1,  15, 300),
	SCROLL("light",                 "STYLO DEBALKAN",       1,  90,  50),
	SCROLL("teleportation",         "POLICY AWESOME",       1,  55, 100),
	SCROLL("gold detection",        "NO YUO",               1,  33, 100),
	SCROLL("food detection",        "MELONPAN",              1,  25, 100),
	SCROLL("identify",              "POKE MANS",            1, 180,  20),
	SCROLL("magic mapping",         "NMBR 3",               1,  45, 100),
	SCROLL("amnesia",               "BRIN GMEM YSAN DWICH", 1,  35, 200),
	SCROLL("fire",                  "TIMOTEI TIMOTEI",      1,  30, 100),
	SCROLL("earth",                 "CLEVAR",               1,  18, 200),
	SCROLL("punishment",            "NEEDSMORE SCIENC",     1,  15, 300),
	SCROLL("charging",              "HACKEM MUCHE",         1,  15, 300),
	SCROLL("stinking cloud",	"VELOX NEB",            1,  15, 300),
	SCROLL((char *)0,		"BOOBIE FLETCH",        1,   0, 100),
	SCROLL((char *)0,		"XKCD",                 1,   0, 100),
	SCROLL((char *)0,		"RND EM KWOT",          1,   0, 100),
...
SPELL("drilling",        "parchment",   P_MATTER_SPELL, 20,  6, 5, 1, RAY,       HI_PAPER),
...
SPELL("buster beam",     "stained",     P_ATTACK_SPELL,  5, 10, 7, 1, RAY,       HI_PAPER),
...
SPELL("knock",           "loganskin",   P_MATTER_SPELL, 35,  1, 1, 1, IMMEDIATE, CLR_BRIGHT_MAGENTA),
...
SPELL("mind control",    "Canadian",    P_ENCHANTMENT_SPELL, 20,  3, 3, 1, IMMEDIATE, CLR_MAGENTA),
...
SPELL("identify",        "bronze",      P_DIVINATION_SPELL, 25,  6, 3, 1, NODIR,     HI_COPPER),
...
SPELL("protection",	 "reverse video", P_CLERIC_SPELL, 18,  3, 1, 1, NODIR,     HI_PAPER),
SPELL("jumping",	 "delicious",   P_ESCAPE_SPELL, 20,  3, 1, 1, IMMEDIATE, HI_PAPER),
...
OBJECT(OBJ("Book of the DESU", "papyrus"), BITS(0,0,1,0,1,0,1,1,0,0,0,P_NONE,PAPER), 0,
...
WAND("light",          "glass",    75, 100, 1, NODIR,     GLASS,    HI_GLASS),
...
WAND("idling",         "oak",      25, 100, 0, IMMEDIATE, WOOD,     HI_WOOD),
...
WAND("evolution",      "nanotube", 30, 200, 1, IMMEDIATE, MINERAL,  HI_MINERAL),
WAND("cancellation",   "platinum", 40, 200, 1, IMMEDIATE, PLATINUM, CLR_WHITE),
WAND("teleportation",  "iridium",  45, 200, 1, IMMEDIATE, METAL,    CLR_BRIGHT_CYAN),
WAND("opening",        "zinc",     20, 150, 1, IMMEDIATE, METAL,    HI_METAL),
WAND("locking",        "aluminum", 20, 150, 1, IMMEDIATE, METAL,    HI_METAL),
WAND("probing ;)",     "uranium",  30, 150, 1, IMMEDIATE, METAL,    HI_METAL),
WAND("drilling",       "CFC",      55, 150, 1, RAY,       IRON,     HI_METAL),
