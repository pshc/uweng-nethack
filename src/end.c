...
boolean ask;
{
	...
	if (nextincted != 0)
	    c = ask ?
	    yn_function("Do you want a list of species eliminated or extinct?",
			ynqchars, defquery) : defquery;
	else
#endif
	...
	if (c == 'y') {
	    klwin = create_nhwindow(NHW_MENU);
#ifdef SHOW_EXTINCT
	    Sprintf(buf, "Eliminated or extinct species:");
#else
	    Sprintf(buf, "Eliminated species:");
#endif
	    putstr(klwin, 0, buf);
	    putstr(klwin, 0, "");
#ifdef DUMP_LOG
