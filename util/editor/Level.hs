...
instance Save Level where
    save lv@(Level {..}) =
              ("\nMAZE:" ++) . save levelName . comma' . save levelFilling
              ...
              . foldr (\r ss -> ss . save r) id levelRegions
              . foldr (\r ss -> ss . save r) id levelSpeRegions
              . foldl (\ss o -> ss . save o) id (concat sortedObjs)
      where
        ...
        sortedObjs = let os  = concat [map ((,) p) ol
                     ...
                     in foldl sortObj acc os
        sortObj acc o = let (bef, (os:aft)) = splitAt (fromEnum (snd o)) acc
                        in bef ++ [o:os] ++ aft
...
saveObj p o = case o of
    ...
    Container ch c s os -> showString "CONTAINER" . chance ch
                           ...
                           . foldr (\o s -> s . saveObj ("contained" ++) o)
                                   id os
  ...
  where
    ...
    objMisc (curs, mon, spe, nm) = commaMaybe curs . commaMaybe mon . comma' .
                                   (if spe >= 0 then ('+':) else id) . save spe
                                   . commaMaybe nm
...
instance Save (Rand Pos) where save = saveRandom save "place"
...
leadZero n | 0 <= n && n < 10 = ('0':) . shows n
           | otherwise        = shows n

instance Save Pos where
    save (x, y) = ('(':) . leadZero x . (',':) . leadZero y . (')':)
instance Save Rect where
    save (a, b, c, d) = ('(':) . leadZero a . (',':) . leadZero b . (',':)
                        . leadZero c . (',':) . leadZero d . (')':)
