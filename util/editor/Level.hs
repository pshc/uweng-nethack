...
loadLevels :: String -> [Level]
loadLevels s = go (lines s)
  where
    go [] = []
    go ls = let (_, start)   = break (isFirstToken "MAZE") ls
                (this, next) = break (isFirstToken "MAZE") (tail start)
            in loadLevel (head start : this) : go next

isFirstToken tok = (== tok) . fst . head . lex

loadLevel :: [String] -> Level
loadLevel = parseLine initLevel
  where
    parseLine lev []     = lev
    parseLine lev (l:ls) = let ts    = getTokens l
                               args_ = dropWhile (/= ":") ts
                               args  = if null args_ then [] else tail args_
      in if null ts then parseLine lev ls else case head ts of
        --"MAZE" -> 
        "MAP" -> let (tiles, ls') = break (isFirstToken "ENDMAP") ls
                     lev'         = lev { levelTiles = readTiles tiles }
                 in parseLine lev' ls'
        --"FLAGS" -> 
        tok -> error ("Unknown token " ++ tok)

    readTiles ls = let w = length (head ls)
                       h = length ls
                   in listArray ((0, 0), (w-1, h-1)) (concat (transpose ls))
